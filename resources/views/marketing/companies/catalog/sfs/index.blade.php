...
<div class="row margin-bottom-20 margin-top-20">
    ...
    <div class="col-md-9">
        ...
        <div class="row">
            <div class="col-md-12">
                <h1>{{ $catalog_description->page_h1 != '' ? $catalog_description->page_h1  : 'Каталог товаров SFS intec' }}</h1>
            </div>
        </div>
        ...
        <ul>
            @foreach($group_categories as $group_category)
            <li><a class="text-primary" href="{{ action('Marketing\Companies\Sfs\CatalogController@getGroup', ['id' => $group_category->id]) }}">{{ $group_category->title }}</a></li>
            @endforeach
        </ul>
    </div>
</div>
