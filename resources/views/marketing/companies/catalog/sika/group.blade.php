...
@section('content')
    <div class="row margin-bottom-20 margin-top-20">
        ...
        <div class="col-md-9">
            ...
            <div class="row">
                <div class="col-md-12">
                    <h1>{{ $group->page_title != '' ? $group->page_h1  : $group->title }}</h1>
                </div>
            </div>
            ...
            @if (count($group->categories) != 0)
                ...
                <ul>
                    @foreach($group->categories as $сategory)
                        <li><a class="text-primary" href="{{ action('Marketing\Companies\Sika\CatalogController@getCategory', ['id' => $сategory->id]) }}">{{ $сategory->title }}</a></li>
                    @endforeach
                </ul>
            @endif
        </div>
